...
snd2wav::snd2wav( const string& inSndFile, const string& inWavFile )
{
	sndFile.open( inSndFile.c_str() );
	wavPath = inWavFile;
}

uint8_t	snd2wav::ReadUint8()
{
	uint8_t		num = 0;
...
void	snd2wav::packV( uint32_t inNum )
{
	wavFile.write( (char*) &inNum, sizeof(inNum) );
}

void	snd2wav::packv( uint16_t inNum )
{
	wavFile.write( (char*) &inNum, sizeof(inNum) );
}

void	snd2wav::print( uint8_t inNum )
{
	wavFile.write( (char*) &inNum, sizeof(inNum) );
}
